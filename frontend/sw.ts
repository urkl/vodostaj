...
//   const data = e.data?.json();
//   if (data) {
//     console.log('Push notification received data:', data); // <-- ADD THIS LINE
//
...
// sw.ts - Service Worker za Vaadin push notifikacije
self.addEventListener('push', (e) => {
  const data = e.data?.json();
  if (data) {
    console.log('Push notification received data:', data);
    const options = {
      body: data.body || 'Ni vsebine.',
      icon: data.icon || '/icons/notification-icon.png',
      badge: data.badge || '/icons/badge.png',
      data: { url: data.url },
      timestamp: data.timestamp || Date.now(),
      actions: [
        { action: 'open_app', title: 'Odpri aplikacijo' }
      ]
    };
    self.registration.showNotification(data.title || 'Obvestilo', options);
  }
});
...
// Poskrbite, da backend pošilja JSON objekt z `title`, `body`, `icon`, `badge`, `url` in `timestamp`.
self.addEventListener('notificationclick', (e) => {
  e.notification.close();
  e.waitUntil(focusOrOpenWindow());
});
